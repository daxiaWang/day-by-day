<template>
  <ul class="entry-list"
    ref="entry">
    <list-item v-for="(item, index) in recommends"
      :key="index"
      :item="item">
      <title-row :title="item.title"></title-row>
      <meta-row :item="item"
        :show-category="true"></meta-row>
    </list-item>
  </ul>
</template>
...
const getData = (store, self) => {
  ...
  return new Promise(resolve => {
    store.dispatch('entries', params).then(res => {
      if (self) {
        const { d, m, s } = res
        if (s === 1) {
          self.recommends.push(...d.entrylist)
        } else {
          self.$message({
            message: m,
            type: 'warning'
